...
function makeRequirable(validate) {
  function checkExists(
    ...
    if (props[propName] === null || props[propName] === undefined) {
      if (isRequired === true) {
        return new Error(
          `The ${location} \`${propFullName}\` is marked as required in \`${componentName}\`, but its value is \`${prop[propName]}\`.`
        )
      }
  ...
  if (
    ...
    return new Error(
      `Invalid prop ${propName} supplied to ${componentName}. ` +
        ...
        `Current values are: \n\tscrollLeft: ${props[propName].scrollLeft} \n\tscrollTop: ${props[propName].scrollTop} \n\tscrollX: ${props[propName].scrollX} \n\tscrollY: ${props[propName].scrollY}`
    )
  }
